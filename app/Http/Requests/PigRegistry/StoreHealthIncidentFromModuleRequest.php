...
class StoreHealthIncidentFromModuleRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $formMode = strtolower(trim((string) $this->input('form_mode', 'general')));
        $formMode = $formMode === '' ? 'general' : $formMode;
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'form_mode' => ['required', 'string', Rule::in(['general', 'mortality'])],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            $formMode = (string) $this->input('form_mode', 'general');

            if ($incidentType === CycleHealthIncident::INCIDENT_TYPE_DECEASED && $formMode !== 'mortality') {
                $validator->errors()->add('incident_type', 'Record deceased incidents using the Mortality form.');
            }

            if ($formMode === 'mortality' && $incidentType !== CycleHealthIncident::INCIDENT_TYPE_DECEASED) {
                $validator->errors()->add('incident_type', 'Mortality form only accepts deceased incidents.');
            }
