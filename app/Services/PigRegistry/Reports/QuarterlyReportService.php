...
class QuarterlyReportService
{
    ...
    public function generate(array $filters): array
    {
        ...
        if (! $start->eq(Carbon::parse($filters['start_date'] ?? $start->toDateString()) ?? $start)) {
            // Only show quarter format if not from custom dates
            $periodLabel = sprintf('Q%d %d', (int) ($filters['quarter'] ?? now()->quarter), (int) ($filters['year'] ?? now()->year));
        }
