<x-app-layout>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form method="POST" action="{{ route('expenses.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            @csrf
            ...
            <div class="p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="expense_date" name="expense_date" type="date" value="{{ old('expense_date', now()->toDateString()) }}" class="w-full rounded-xl border-gray-200 focus:border-[#0c6d57] focus:ring-[#0c6d57]" required>
                    @error('expense_date')
                ...
                <div>
                    ...
                    <select id="batch_id" name="batch_id" class="w-full rounded-xl border-gray-200 focus:border-[#0c6d57] focus:ring-[#0c6d57]" required>
                        ...
                        @foreach ($cycles as $cycle)
                            <option value="{{ $cycle->id }}" @selected(old('batch_id', $selectedCycleId) == (string) $cycle->id)>
                                {{ $cycle->batch_code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="amount" name="amount" type="number" step="0.01" min="0.01" value="{{ old('amount') }}" placeholder="0.00" class="w-full rounded-xl border-gray-200 focus:border-[#0c6d57] focus:ring-[#0c6d57]" required>
                    @error('amount')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3" class="w-full rounded-xl border-gray-200 focus:border-[#0c6d57] focus:ring-[#0c6d57]" placeholder="Example: 3 sacks of grower feeds." required>{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2" x-data="{ fileName: '' }">
                    <label for="receipt" class="block text-sm font-semibold text-gray-700 mb-1">Receipt (Optional)</label>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                <a href="{{ route('expenses.index') }}" class="inline-flex justify-center items-center px-5 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                    Cancel
                </a>
