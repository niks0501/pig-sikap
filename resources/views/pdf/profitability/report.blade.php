...
<body>
    @php
        // DomPDF can fail to render the peso glyph depending on font availability.
        // Use a stable ASCII currency prefix to avoid "?123.45" output in reports.
        $currencyPrefix = 'PHP ';
    @endphp
    ...
    @if ((float) $profitability['net_profit_or_loss'] < 0)
        <div class="loss-notice">
            ...
            This cycle recorded a net loss. All stakeholder shares are {{ $currencyPrefix }}0.00. No distributable profit is available.
        </div>
    @endif
    ...
    <table>
        <tr><th>Total Sales (Gross Revenue)</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['total_sales'], 2) }}</td></tr>
        <tr><th>Total Collected</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) ($profitability['total_collected'] ?? 0), 2) }}</td></tr>
        <tr><th>Receivables (Pending Collection)</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) ($profitability['receivables'] ?? 0), 2) }}</td></tr>
        <tr><th>Total Expenses</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['total_expenses'], 2) }}</td></tr>
        <tr class="total-row"><th>Net Profit / Loss</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['net_profit_or_loss'], 2) }}</td></tr>
    </table>
    ...
    <table>
        @foreach ($profitability['expense_breakdown_rows'] as $row)
            @if ((float) $row['total'] > 0)
                <tr>
                    <td>{{ $row['label'] }}</td>
                    <td class="right">{{ $currencyPrefix }}{{ number_format((float) $row['total'], 2) }}</td>
                </tr>
            @endif
        @endforeach
        <tr class="total-row">
            <th>Total Expenses</th>
            <td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['total_expenses'], 2) }}</td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tr class="total-row">
            <th>Gross Revenue</th>
            <td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['total_sales'], 2) }}</td>
        </tr>
    </table>
    ...
    <table>
        <tr><th>Distributable Profit</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['distributable_profit'], 2) }}</td></tr>
        <tr><th>Caretaker / Nag-alaga (50%)</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['caretaker_share'], 2) }}</td></tr>
        <tr><th>Association Members (25%)</th><td class="right">{{ $currencyPrefix }}{{ number_format((float) $profitability['member_share'], 2) }}</td></tr>
