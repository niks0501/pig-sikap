...
<body>
    <div class="header">
        <div class="badge">HEALTH REPORT</div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Due Today</th>
                <th>Overdue</th>
                <th>Completed Recently</th>
                <th>Currently Affected</th>
        ...
        <tbody>
            @foreach ($report['rows'] ?? [] as $row)
                <tr>
                    ...
                    <td>{{ $row['due_today'] ?? 0 }}</td>
                    <td>{{ $row['overdue'] ?? 0 }}</td>
                    <td>{{ $row['completed_recently'] ?? 0 }}</td>
                    <td>{{ $row['currently_affected'] ?? 0 }}</td>
                    <td>{{ $row['total_incidents'] ?? 0 }}</td>
                    <td>{{ $row['mortality'] ?? 0 }}</td>
                </tr>
            @endforeach
            <tr class="total-row">
                <td><strong>Totals</strong></td>
                <td><strong>{{ $summary['total_due_today'] ?? 0 }}</strong></td>
                <td><strong>{{ $summary['total_overdue'] ?? 0 }}</strong></td>
                <td><strong>{{ collect($report['rows'] ?? [])->sum('completed_recently') }}</strong></td>
                <td><strong>{{ $summary['total_currently_affected'] ?? 0 }}</strong></td>
                <td></td>
                <td><strong>{{ $summary['total_mortality'] ?? 0 }}</strong></td>
            </tr>
        </tbody>
    </table>

    @if (!empty($chartImages['healthStatus'] ?? null))
        <div style="margin-top: 20px; page-break-inside: avoid;">
            <div style="font-size: 13px; font-weight: bold; color: #0c6d57; margin-bottom: 8px;">Health Status Overview</div>
            <img src="{{ $chartImages['healthStatus'] }}" style="max-width: 100%;">
        </div>
    @endif
